...
dissim = pdist(binary_matrix.T)
z = pd.DataFrame(squareform(dissim))
z = np.fill_diagonal(squareform(dissim), 99999999)


#%%
cps_accurate = pd.DataFrame(cps)
cps_accurate["accuracy"] = stripped_ascs
cps_accurate = cps_accurate[cps_accurate["accuracy"] > 0]
count = 0
ascs_of_correct = []
test = pd.DataFrame()
...
" ".join(words_matrix.iloc[:, cp[0]].str.cat(sep=" ").split())
